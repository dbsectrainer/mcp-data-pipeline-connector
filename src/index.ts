...
async function main(): Promise<void> {
  ...
  const readOnly = argv["read-only"] as boolean;
  const maxRows = argv["max-rows"] as number;
  const configPath = argv["config"] as string | undefined;
  const restCacheTtl = argv["rest-cache-ttl"] as number;
  const httpPort = argv["http-port"] as number;
...
main().catch((err: unknown) => {
  process.stderr.write(`[mcp-data-pipeline-connector] Fatal error: ${(err as Error).message}\n`);
  process.exit(1);
});
