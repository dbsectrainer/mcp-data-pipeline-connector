...
function buildTransformSql(
  ...
  let aggregateExpr: string | undefined;
    ...
    switch (op.type) {
      ...
      case "rename": {
        ...
        if (mappings && typeof mappings === "object" && !Array.isArray(mappings)) {
          renameMap = mappings as Record<string, string>;
        }
        break;
      }
      case "aggregate": {
...
export function createServer(options: ServerOptions): {
   ...
   */
  function emitProgress(progressToken: string | number, progress: number, total: number): void {
    try {
      void server.notification({
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
    ...
    try {
      switch (name) {
        ...
        case "query": {
          ...
          const isCrossSource =
            ...
            (Array.isArray(toolArgs["sources"]) && (toolArgs["sources"] as string[]).length > 1);
        ...
        case "transform": {
          ...
          const outputFormat = (toolArgs["output_format"] as string | undefined) ?? "json";
      ...
      throw new McpError(ErrorCode.InternalError, `Internal error: ${(err as Error).message}`);
    }
  });
 ...
 */
export async function autoLoadSources(
  registry: SourceRegistry,
  configPath?: string,
): Promise<void> {
  try {
    ...
    for (const source of config.sources) {
      try {
        ...
        process.stderr.write(
          `[server] Failed to auto-load source '${source.name}': ${(err as Error).message}\n`,
        );
      }
    }
    process.stderr.write(`[server] Auto-loaded ${count} source(s) from config\n`);
  } catch (err) {
    process.stderr.write(`[server] Failed to load config: ${(err as Error).message}\n`);
  }
}
