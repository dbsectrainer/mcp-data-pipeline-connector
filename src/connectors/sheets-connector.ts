 ...
 */
function buildServiceAccountJwt(sa: ServiceAccount): string {
 ...
 */
async function httpsGet(url: string, accessToken: string): Promise<unknown> {
...
export class SheetsConnector implements DataConnector {
  readonly name: string;
  readonly type = "sheets";

  private config: SheetsConfig & { name: string };
  private connected = false;
  private accessToken: string | null = null;

  constructor(config: SheetsConfig & { name: string }) {
    this.name = config.name;
    this.config = config;
  }

  async connect(config?: SheetsConfig): Promise<void> {
  ...
  async listTables(): Promise<string[]> {
    ...
    const data = (await httpsGet(url, this.accessToken)) as {
