    ...
    async listTables(sourceName) {
        ...
        for (const [name, connector] of this.sources.entries()) {
            try {
                ...
                process.stderr.write(`[source-registry] Failed to list tables for '${name}': ${err.message}\n`);
            }
        }
        return results;
    }
    ...
    async getCrossSourceDb() {
        ...
        for (const [name, connector] of this.sources.entries()) {
            if (connector.type === "csv" && connector.isConnected()) {
                ...
                try {
                    await connection.run(`CREATE VIEW "${viewName}" AS SELECT * FROM read_csv_auto('${resolvedPath}')`);
                }
                catch (err) {
                    process.stderr.write(`[source-registry] Failed to attach CSV source '${name}' to cross-source DB: ${err.message}\n`);
                }
            }
        }
