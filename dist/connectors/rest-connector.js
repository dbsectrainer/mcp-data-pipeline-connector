...
export class RestConnector {
    ...
    cacheTtlMs;
    cache = new Map();
    constructor(config, globalCacheTtl) {
        if (!config.url) {
    ...
    async connect() {
        ...
        try {
            ...
            this.connected = true;
            process.stderr.write(`[rest-connector] Connected '${this.name}' -> ${sanitizeUrl(this.baseUrl)}\n`);
        }
        catch (err) {
            this.connected = false;
            throw new Error(`Failed to connect REST source '${this.name}': ${err.message}`);
        }
    }
    async disconnect() {
        this.cache.clear();
    ...
    async query(sql, maxRows) {
        ...
        try {
            ...
            const jsonString = JSON.stringify(Array.isArray(rawData) ? rawData : [rawData]).replace(/'/g, "''");
            ...
            await connection.run(`CREATE TABLE "response" (data VARCHAR); INSERT INTO "response" VALUES ('${escaped}')`);
        }
