...
export class PostgresConnector {
    ...
    async connect() {
        ...
        try {
            ...
            await this.connection.run(`ATTACH '${connString}' AS "${this.attachedAlias}" (TYPE POSTGRES)`);
            this.connected = true;
            process.stderr.write(`[postgres-connector] Connected '${this.name}' -> ${sanitizeConnectionString(connString)}\n`);
        }
        catch (err) {
    ...
    async listTables() {
        if (!this.connection)
            throw new Error(`Source '${this.name}' is not connected`);
        const result = await this.connection.run(`SELECT table_name FROM "${this.attachedAlias}".information_schema.tables WHERE table_schema = 'public' ORDER BY table_name`);
    ...
    async getSchema(tableName) {
        if (!this.connection)
            throw new Error(`Source '${this.name}' is not connected`);
        const result = await this.connection.run(`DESCRIBE SELECT * FROM "${this.attachedAlias}"."${tableName}"`);
        const rows = await result.getRows();
        return rows.map((row) => {
