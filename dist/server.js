...
import { CallToolRequestSchema, CancelledNotificationSchema, ListToolsRequestSchema, ListResourcesRequestSchema, ReadResourceRequestSchema, ListPromptsRequestSchema, GetPromptRequestSchema, McpError, ErrorCode, } from "@modelcontextprotocol/sdk/types.js";
    ...
    for (const op of operations) {
        switch (op.type) {
            case "filter": {
                ...
                const safeValue = typeof value === "string" ? `'${value.replace(/'/g, "''")}'` : String(value ?? "NULL");
...
export function createServer(options) {
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => {
        return {
            tools: [
                {
                    name: "connect_source",
                    description: "Register a data source (CSV file, Postgres database, or REST API). " +
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            ...
                            source_config_path: {
                                type: "string",
                                description: "Path to a YAML config file. If provided, all sources in the file are registered.",
                            },
                        },
                        required: ["name", "type"],
                    },
                ...
                {
                    name: "query",
                    description: "Execute a SQL query against a registered data source using DuckDB. " +
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            ...
                            source: {
                                type: "string",
                                description: "Name of the source to query, or '_all' for cross-source joins. Required if multiple sources are registered.",
                            },
                            sources: {
                                type: "array",
                                items: { type: "string" },
                                description: "Array of source names for cross-source queries (alternative to source='_all')",
                            },
                            max_rows: {
                                type: "number",
                ...
                {
                    name: "transform",
                    description: "Apply aggregations, filters, renaming, or column selection to a source table and return or save results.",
                    annotations: { readOnlyHint: true },
                    inputSchema: {
                        type: "object",
                        properties: {
    ...
    server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
        ...
        if (!match) {
            throw new McpError(ErrorCode.InvalidParams, `Invalid resource URI: ${uri}. Expected format: data://{source_name}/{table_name}`);
        }
    ...
    server.setRequestHandler(ListPromptsRequestSchema, async () => {
        return {
            prompts: [
                {
                    name: "explore-data",
                    description: "Guide agents through schema discovery and querying. " +
            ...
            messages: [
                {
                    ...
                    content: {
                        type: "text",
                        text: `You are exploring a data pipeline.${sourceHint} Follow these steps in order:\n\n` +
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
        ...
        try {
            switch (name) {
                ...
                case "list_tables": {
                    const toolArgs = args;
                    const sourceName = toolArgs && typeof toolArgs["source"] === "string" ? toolArgs["source"] : undefined;
                ...
                case "get_schema": {
                    ...
                    const connector = registry.get(sourceName);
                    if (!connector) {
                        throw new McpError(ErrorCode.InvalidParams, `Source '${sourceName}' not found. Use list_sources to see available sources.`);
                    }
                    const schema = await connector.getSchema(tableName);
                    return {
                        content: [
                ...
                case "query": {
                    ...
                    if (!sql || typeof sql !== "string" || sql.trim() === "") {
                        throw new McpError(ErrorCode.InvalidParams, "sql is required and must be a non-empty string");
                    }
                    ...
                    // Determine max rows
                    const maxRows = toolArgs["max_rows"] && typeof toolArgs["max_rows"] === "number"
                    ...
                    const isCrossSource = toolArgs["source"] === "_all" ||
                    ...
                    try {
                        ...
                        if (isCrossSource) {
                            ...
                            try {
                                if (complex)
                                    emitProgress(progressToken, 30, 100);
                                // Apply pagination
                                ...
                                if (complex)
                                    emitProgress(progressToken, 60, 100);
                            ...
                            if (toolArgs["source"] && typeof toolArgs["source"] === "string") {
                                ...
                                if (!connector) {
                                    throw new McpError(ErrorCode.InvalidParams, `Source '${toolArgs["source"]}' not found. Use list_sources to see available sources.`);
                                }
                            }
                            else {
                                const sources = registry.list();
                                if (sources.length === 0) {
                                    throw new McpError(ErrorCode.InvalidParams, "No sources registered. Use connect_source first.");
                                }
                                if (sources.length > 1) {
                                    throw new McpError(ErrorCode.InvalidParams, `Multiple sources are registered. Specify which one to query using the 'source' parameter. Available: ${sources.map((s) => s.name).join(", ")}`);
                                }
                    ...
                    if (complex)
                        emitProgress(progressToken, 100, 100);
                    return {
                        content: [
                            {
                                type: "text",
                ...
                case "transform": {
                    ...
                    if (requestId !== undefined)
                        activeRequests.set(requestId, transformAbortController);
                    let rawResult;
                    try {
                    ...
                    if (outputFormat === "csv") {
                        ...
                        const rows = rawResult.rows.map((row) => rawResult.columns
                            .map((col) => {
                            const val = row[col];
                    ...
                    emitLog("debug", `Transform executed: ${operations.length} operation(s) on ${sourceName}.${tableName}`);
                    return {
                        content: [
                            {
                                type: "text",
                ...
                case "check_health": {
                    ...
                    for (const r of results) {
                        if (!r.healthy) {
                            emitLog("warning", `Health check failed for source '${r.name}': ${r.error ?? "unknown error"}`);
                        }
                    }
                    return {
                        content: [
    ...
    try {
        ...
        for (const source of config.sources) {
            try {
                ...
                process.stderr.write(`[server] Failed to auto-load source '${source.name}': ${err.message}\n`);
            }
        }
