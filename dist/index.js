...
async function main() {
    const argv = await yargs(hideBin(process.argv))
        ...
        type: "number",
        default: 60,
        description: "Time-to-live in seconds for cached REST API responses. Set to 0 to disable caching. Default: 60.",
    })
        .option("http-port", {
        type: "number",
        default: 0,
        description: "Port to expose the MCP server over Streamable HTTP transport (MCP 2025 spec). " +
    ...
    process.stderr.write(`[mcp-data-pipeline-connector] Starting (read-only=${readOnly}, max-rows=${maxRows}, rest-cache-ttl=${restCacheTtl}s)\n`);
    const serverOptions = {
        readOnly,
        maxRows,
    ...
    if (httpPort > 0) {
        ...
        process.stderr.write(`[mcp-data-pipeline-connector] Starting HTTP transport on port ${httpPort}\n`);
